        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                @if ($task->isOnClientReview())
                    <p class="small text-danger mb-0">Reminders are paused while this task is on client review. No emails go to developers or admin.</p>
                @elseif ($task->developerEmails() === [])
            ...
            <div class="modal-footer flex-wrap gap-2">
                <form method="POST" action="{{ route('tasks.reminders.store', $task) }}">
                    ...
                    <button class="btn btn-dark" type="submit" @disabled($task->isOnClientReview() || $task->developerEmails() === [])>Email reminder</button>
                </form>
                @if ($askOnLoad ?? false)
